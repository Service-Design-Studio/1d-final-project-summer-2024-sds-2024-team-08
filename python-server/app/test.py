...
#     Returns:
#         relationships_with_predicates (dict(edges: list[list[int, str, int]], nodes: dict(int:str)): A dictionary with key 'edges' and 'nodes'. 
#             relationships_with_predicates['edges'] is A list of subjects, predicates and objects. 
...
#             relationships_with_predicates['nodes'] is a dictionary where the key is the stakeholder_id and the value is the name of the stakeholder.
#     '''
#     # print("Getting relationships")
#     subject_id = int(float(subject_id))
#     relationships_with_predicate = {
#         "edges": [],
#         "nodes": {}
#     }
#     with Session(stakeholder_engine) as session:
...
#         return relationships_with_predicate 
...
#         relationships_with_predicate["edges"].append((result.subject, extracted_info, result.object))
#         with Session(stakeholder_engine) as session:
#             if result.subject not in relationships_with_predicate["nodes"]:
#                 relationships_with_predicate["nodes"][result.subject] = crud.get_stakeholder_name(session, result.subject)
#             if result.object not in relationships_with_predicate["nodes"]:
#                 relationships_with_predicate["nodes"][result.object] = crud.get_stakeholder_name(session, result.object)
...
def generate_tools(chat_id: int) -> List[BaseTool]:
    ...
    def generate_network(relationships: list[list[str]]) -> dict:
        ...
        Returns:
            results (str): A JSON object containing the message 'network graph has been created and stored in the database!.'
        """
        ...
        print(network_graph)
        print("=================")
        return {"message": "Network graph has been created and stored in the database!"}
...
def query_model(query:str, user_id:int, chat_id:int) -> str:
    ...
    You are working in a team and if you deem that your reply is not yet complete but you cannot progress it anymore, it is okay. The next agent in the workflow will continue the conversation and improve the answer. Your team member is 'Graph_Master' who will help you generate the graph once you have identified the stakeholders and relationships.
    You are able to extract the relationships that a stakeholder has if requested.
    ...
    You can use these tools to help you identify the appropriate relationships: get_relationships.
    ...
    ## 4. Graph_Master
    When a user asks for a network graph, you must pass the relationships you identified using the get_relationships tool to the Graph_Master agent to generate a network graph based on the relationships of the stakeholders that you have identified.
    In order to pass your response to the Graph_Master agent, you must prefix your reponse with 'GRAPH_MASTER'.
    ## 5. End
    If you think that you do not need to pass your response to Graph_Master, you can end the conversation by prefixing FINAL ANSWER.
    """
    
    researcher_tools = [read_stakeholders, get_name_matches, get_relationships]
    researcher_agent = custom_workflow.create_agent(model, researcher_tools, system_message=researcher_system_message)
    researcher_node = functools.partial(custom_workflow.agent_node, agent=researcher_agent, name="Researcher")    
    
    graph_master_system_message = """
    ## 1. Overview
    You are a top-tier algorithm designed to generate network graphs to show the relationships of stakeholder graphically. Your aim is to generate the graph and upload it to the database.
    You are working in a team and if you deem that your reply is not yet complete but you cannot progress it anymore, it is okay. The next agent in the workflow will continue the conversation and improve the answer. Your team member is 'Researcher' who will help you identify the stakeholders and relationships.
    You are able to build a network graph based on the relationships you receive from the agent, 'Researcher'.
    You must capture the user's input and provide the appropriate response based on the user's query. 
    The aim is to provide the user with the most relevant information based on the user's query.
    ## 2. Network Graph
    ...
    Once you have generated the network graph, you will store the graph in the database and return 'The network graph has been created and stored in the database!' to the user. Once you have stored the graph, you job is done!
    ## 3. End
    If you think that the response is final and you can no longer progress the response, you can end the conversation by prefixing FINAL ANSWER.
    """
    
    graph_master_tools = generate_tools(chat_id=chat_id)
    graph_master_agent = custom_workflow.create_agent(model, graph_master_tools, system_message=graph_master_system_message)
    graph_master_node = functools.partial(custom_workflow.agent_node, agent=graph_master_agent, name="Graph_Master")
    
    tools = researcher_tools + graph_master_tools  
    tool_node = ToolNode(tools)

    graph = custom_workflow.create_workflow(model, researcher_node, graph_master_node, tool_node, checkpointer=PostgreSQLMemorySaver(engine=user_engine))
    ...
    response_str = events['messages'][-1].content
    print(4) 
...
if __name__ == '__main__':
    print(query_model("Generate a network graph to visualize graphically the relationships of Ben Carson to other stakeholders.", 3, 12))
    # print(query_model("tell me aout Ben Carson", 3, 15))
    # print(query_model("Give me a summary about Ben Carson, Donald Trump and Joe Biden", 3, 17))
    # print(query_model("Using these relationships: [[1.0, 'MBA in Psychology', 29155.0], [1.0, 'Director', 3656.0], [1.0, 'Education', 29155.0]]. Remember to convert the relationships I sent you into a list first. Right now, it is a string. generate a network graph to visualize graphically the relationships of Ben Carson to other stakeholders.", 3, 13))
...
def read_stakeholders(stakeholder_id: int = None, name: str = None, summary: bool = True, headline: bool = True, photo: bool = True) -> bytes:
    """Use this tool to access and know more about stakeholders information from the database. You can filter by stakeholder_id or name. From the prompt, identify the stakeholder by their name or stakeholder_id. 
...
def get_relationships_with_names(subject_id:int = None) -> bytes:
    ...
    with Session(stakeholder_engine) as session:
        subject_rs = crud.get_relationships_with_names(session, subject=subject_id) #outgoing relationships
        object_rs = crud.get_relationships_with_names(session, object=subject_id) #incoming relationships

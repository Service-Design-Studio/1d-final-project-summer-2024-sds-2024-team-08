...
def query_model(query:str, user_id:int, chat_id:int) -> str:
    ...
    ## 4. Graph_Master
    When a user asks for a network graph, you must pass the relationships to the Graph_Master agent to generate a network graph based on the relationships of the stakeholders that you have identified."""
    ...
    ## 1. Overview
    You are a top-tier algorithm designed to generate network graphs to show the relationships of stakeholder graphically.
...
# , checkpointer=PostgreSQLMemorySaver(engine=user_engine)
    graph = custom_workflow.create_workflow(model, researcher_node, graph_master_node, tool_node)
    ...
    # response_str = response[-1]['messages'][-1].content
    print(4) 
...
if __name__ == '__main__':
    print(query_model("Generate a network graph to visualize graphically the relationships of Ben Carson to other stakeholders.", 3, 16))